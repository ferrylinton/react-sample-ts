            ...
            <nav className='flex-none fixed top-0 left-0 w-full h-[50px] bg-white border-b border-slate-300 flex items-center px-2 z-50 shadow-md shadow-gray-200 '>
                ...
                <div className='ml-auto flex items-center gap-2'>
                    <div className={`nav-menu ${isOpen ? 'open' : 'close'}`}>
                        <Link onClick={() => setIsOpen(false)} to='/'>{intl.formatMessage({ id: "home" })}</Link>
                        <Link onClick={() => setIsOpen(false)} to='/post'>{intl.formatMessage({ id: "post" })}</Link>
                        <Link onClick={() => setIsOpen(false)} to='/tag'>{intl.formatMessage({ id: "tag" })}</Link>
                        <Link onClick={() => setIsOpen(false)} to='/about'>{intl.formatMessage({ id: "about" })}</Link>
                    </div>

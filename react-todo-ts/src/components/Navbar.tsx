    ...
    return (
        <nav className='flex-none fixed top-0 left-0 w-full h-[50px] bg-white border-b border-slate-300 flex items-center justify-center px-2 z-50 shadow-md shadow-gray-200 '>
            <div className='w-full md:max-w-2xl lg:max-w-4xl xl:max-w-6xl flex items-center justify-center'>
                <div className='text-2xl font-bold uppercase'><FormattedMessage id="app.name" defaultMessage={'Todo'} /></div>
                <div className='ml-auto flex items-center gap-2'>
                    <div className={`nav-menu ${isOpen ? 'open' : 'close'}`}>
                        <Link onClick={() => setIsOpen(false)} to='/'><FormattedMessage id="home" defaultMessage={'Home'} /></Link>
                        <Link onClick={() => setIsOpen(false)} to='/about'><FormattedMessage id="about" defaultMessage={'About'} /></Link>
                        <Link onClick={() => setIsOpen(false)} to='/author'><FormattedMessage id="author" defaultMessage={'Author'} /></Link>
                    </div>
